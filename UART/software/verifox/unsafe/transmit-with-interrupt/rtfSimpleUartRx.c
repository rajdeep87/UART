            ...
            if((unsigned int)state_old == 1)
            {
              if(cnt_old == 151)
              {
                ...
                *overrun = *data_present;
                ...
                if(!(*data_present))
                  srtfSimpleUartRx.wf = 0; // bug: wf must be 1 here to make data present high in next cycle

                srtfSimpleUartRx.state = 1; // bug: state must transit to 0 here
              }
